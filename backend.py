...
def search(folder_link):
    global driver
    folder = folder_link
    print(folder)
    # 크롬 실행
    driver = webdriver.Chrome(folder)
    driver.get('https://shop.palaceskateboards.com/collections/new')
    driver.maximize_window()  # 윈도우화면 최대화


def start(item,size,email,firstname,lastname,address,apartment,city,postcode,phone):
    global driver
    print(item,size,email,firstname,lastname,address,apartment,city,postcode,phone)
    # item = window.entry0.get().upper()
    # size = window.entry1.get()  # 리스트
    # email = window.entry2.get()  # 텍스트
    # firstname = window.entry3.get()  # 텍스트
    # lastname = window.entry4.get()  # 텍스트
    # address = window.entry5.get()  # 텍스트
    # apartment = window.entry6.get()  # 텍스트
    # city = window.entry7.get()  # 텍스트
    # postcode = window.entry8.get()  # 텍스트
    # phone = window.entry9.get()  # 텍스트
